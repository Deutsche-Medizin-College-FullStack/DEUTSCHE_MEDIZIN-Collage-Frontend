...
export default function ViceDepartmentDetail() {
  ...
  const fetchDepartmentDetails = async () => {
    try {
      ...
      const deptResponse = await apiService.get(endPoints.getDepartmentById(id));
        ...
        {courses.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">
              Course information for this department is not available in the current API.
            </p>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">
              Note: The department API endpoint provides basic department information
              but not course details. Course data would need to be fetched from a
              separate endpoint if available.
            </p>
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow border border-gray-200 dark:border-gray-700">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Department Code:</span>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{department.programLevel?.name}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Program Level Code:</span>
              <span className="font-medium">{department.programLevel?.code}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow border border-gray-200 dark:border-gray-700">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Modality:</span>
              <span className="font-medium">{department.programModality?.modality}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Modality Code:</span>
              <span className="font-medium">{department.programModality?.modalityCode}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Total Credit Hours:</span>
              <span className="font-medium">
                {department.totalCrHr !== null ? department.totalCrHr : "Not Specified"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Program Active:</span>
              <span className={`font-medium ${department.programLevel?.active ? 'text-green-600' : 'text-red-600'}`}>
